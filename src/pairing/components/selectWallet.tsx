...
import { Button, Comment, Header, Image, Message } from 'semantic-ui-react'
  ...
  render() {
    ...
    const chainsLabel =
      p.chains.length > 0 ? (
        <>
          {p.chains.map((x, i) =>
            i !== p.chains.length - 1 ? <b key={x}>{x}, </b> : <b key={x}>{x}</b>
          )}{' '}
          chain{p.chains.length > 1 ? 's' : null}
        </>
      ) : (
        <>
          <b>any</b> chain
      ...
      <>
        ...
        {disconnectedWallets.length > 0 ? (
          <>
            ...
            {!this.state.descriptors.find((x) => x.type === WalletTypes.METAMASK).connected &&
            (p.chains.length === 0 || p.chains.includes(ChainTypes.ETHEREUM_GOERLI)) ? (
            ...
            {!this.state.descriptors.find((x) => x.type === WalletTypes.WALLETCONNECT).connected &&
            (p.chains.length === 0 || p.chains.includes(ChainTypes.ETHEREUM_GOERLI)) ? (
              <Button
            ...
            {!this.state.descriptors.find(
              (x) => x.type === WalletTypes.NEAR && x.chain === ChainTypes.NEAR_TESTNET
            ).connected &&
            (p.chains.length === 0 || p.chains.includes(ChainTypes.NEAR_TESTNET)) ? (
              <Button
                // disabled={this.state.descriptors.find(x => x.type === 'walletconnect').available === false}
                basic
                fluid
            ...
            {!this.state.descriptors.find(
              (x) => x.type === WalletTypes.NEAR && x.chain === ChainTypes.NEAR_MAINNET
            ).connected &&
            (p.chains.length === 0 || p.chains.includes(ChainTypes.NEAR_MAINNET)) ? (
              <Button
                // disabled={this.state.descriptors.find(x => x.type === 'walletconnect').available === false}
                basic
                fluid
            ...
            {!this.state.descriptors.find((x) => x.type === WalletTypes.DAPPLETS).connected &&
            (p.chains.length === 0 || p.chains.includes(ChainTypes.ETHEREUM_GOERLI)) ? (
              <Button
                // disabled={this.state.descriptors.find(x => x.type === 'walletconnect').available === false}
                basic
