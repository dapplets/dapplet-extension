...
export class IpfsModuleStorage implements ModuleStorage {
    ...
    public async getResource(uri: string, fetchController: AbortController = new AbortController()): Promise<ArrayBuffer> {
        ...
        const response = await timeoutPromise(
            this.timeout,
            fetch(joinUrls(this._gateway, "ipfs/" + this._extractReference(uri)), { signal: fetchController.signal }),
    ...
    private _extractReference(uri: string) {
        const result = uri.match(/Qm[1-9A-HJ-NP-Za-km-z]{44,}|b[A-Za-z2-7]{58,}|B[A-Z2-7]{58,}|z[1-9A-HJ-NP-Za-km-z]{48,}|F[0-9A-F]{50,}/gm);
