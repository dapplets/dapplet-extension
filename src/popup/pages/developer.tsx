...
class Developer extends React.Component<IDeveloperProps, IDeveloperState> {
  ...
  async loadRegistries() {
    ...
    const modules: { module: ModuleInfo; versions: VersionInfo[]; isDeployed: boolean[] }[] =
  ...
  render() {
    ...
    return (
      <>
        {intro.popupDeveloperWelcome ? (
          <Message info onDismiss={() => this.closeWelcomeIntro()}>
            <Message.Header>Welcome to Development Mode!</Message.Header>
            <p>
              Here you can connect to development servers to debug modules, publish them to public
              registries using your wallet.
            </p>
          </Message>
        ) : null}
        ...
        >
          ...
          >
            ...
            <Input
              ...
              value={registryInput}
              onChange={(e) =>
                this.setState({ registryInput: e.target.value, registryInputError: null })
              }
                ...
                <List.Item key={i}>
                  <List.Content floated="left">
                    {r.isEnabled ? (
                      !r.error ? (
                        <HoverLabel
                          style={{ cursor: 'pointer', width: '56px', textAlign: 'center' }}
                          size="mini"
                          horizontal
                          color="green"
                          hoverText="DISABLE"
                          hoverColor="red"
                          onClick={() => this.disableRegistry(r.url)}
                        >
                          ENABLED
                        </HoverLabel>
                      ) : (
                  ...
                  <List.Content>
                    <a style={{ color: '#000' }} onClick={() => window.open(r.url, '_blank')}>
                      {r.url}
                    </a>
                  </List.Content>
                </List.Item>
                ...
                Object.entries(groupedModules).map(([registryUrl, modules]) => (
                  <div style={{ marginBottom: '1.5em' }} key={registryUrl}>
                    ...
                    {modules.length > 0 ? (
                      <DevModulesList
                        modules={modules}
                        onDetailsClick={this.deployModule.bind(this)}
                      />
                    ) : (
