...
export class Login extends React.Component<Props, State> {
    ...
    async loadData() {
        ...
        const descriptorsAndIdentities: (WalletDescriptor & { relatedIdentities: Account[] })[] = await Promise.all(descriptors.filter(x => x.connected && x.account)
            .map(x => getIdentityAccounts({ domainId: DomainTypes.Ethereum, name: x.account.toLowerCase() }).then(y => ({ ...x, relatedIdentities: y }))));
        const suitable = descriptorsAndIdentities.filter(x => x.relatedIdentities.find(y => y.domainId === p.account.domainId && y.name.toLowerCase() === p.account.username.toLowerCase()));
                    ...
                    <Comment.Group>
                        {connectedWallets.map((x, i) => (
                            <div key={i} style={{ marginBottom: '10px', display: 'flex', boxShadow: '0 0 0 1px rgba(34,36,38,.15) inset', borderRadius: '.28571429rem', padding: '.78571429em 1.5em .78571429em' }}>
                                ...
                                <div>
                                    <Button primary onClick={() => this.selectWallet(x.type)} size='tiny' style={{ margin: '5px 0' }}>Login</Button>
                                </div>
                            </div>
                        ))}
