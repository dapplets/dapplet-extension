...
export class Connection<T> implements IConnection {
  ...
  constructor(private _bus?: IPubSub, private _eventDef?: EventDef<any>) {
  ...
  listen(h: MsgHandler | EventHandler): this {
    console.warn(
      'DEPRECATED: "listen" method of the Connection class is deprecated since version 0.46.1. Will be deleted in version 0.50.0. Use "declare" instead'
    )
  ...
  private listener(
    filterOrHander?: MsgFilter | EventHandler,
    evtOrMsgOrApiOrAP?: EventHandler | MsgHandler | IDappletApi | AutoProperty[]
  ): Listener {
    let listener: Listener

    if (filterOrHander === undefined) {
      listener = { f: undefined, h: undefined, p: [] }
    } else if (typeof filterOrHander === 'object' && !filterOrHander.then) {
      //is an EventHandler
      listener = { f: undefined, h: filterOrHander as EventHandler, p: [] }
    } else if (typeof filterOrHander === 'object') {
      // is an Promise
  ...
  onMessage(op: any, msg: any): void {
    try {
      ...
      this._listeners.forEach(async (listener) => {
        ...
        if (
          !listener.f ||
          listener.f === 'dappletApi' ||
          listener.f === 'dappletApiClass' ||
          isTopicMatch(op, msg, listener.f)
        ) {
          if (listener.h) {
            const getAllApiNames = (dappletApi: any) => {
              ...
              const allApiClassNames = props.filter(
                (e, i, arr) => e != arr[i + 1] && typeof dappletApi[e] == 'function'
              )
              const objBaseMethods = [
                '__defineGetter__',
                '__defineSetter__',
                '__lookupGetter__',
                '__lookupSetter__',
            ...
            const eventsIds =
              listener.f === 'dappletApiClass'
                ? [...getAllApiNames(listener.h), ANY_EVENT]
                : [...Object.keys(listener.h), ANY_EVENT]
            if (
              (listener.f === 'dappletApi' || listener.f === 'dappletApiClass') &&
              !eventsIds.includes(msg?.type)
            ) {
            ...
            for (const eventId of eventsIds) {
              ...
              if (
                (typeof cond === 'function' ? cond(op, msg) : msg?.type == cond) ||
                eventId === ANY_EVENT
              ) {
