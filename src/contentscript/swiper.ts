export class Swiper {
  ...
  private _touchHandler() {
    ...
    if (
      90 - 45 * angleWidth < angle &&
      angle < 90 + 45 * angleWidth &&
      Math.abs(dY) / height > lengthPart
    ) {
      this._fireEvent('up')
    }

    if (
      ((180 - 45 * angleWidth < angle && angle < 180) ||
        (-180 < angle && angle < -180 + 45 * angleWidth)) &&
      Math.abs(dX) / width > lengthPart
    ) {
